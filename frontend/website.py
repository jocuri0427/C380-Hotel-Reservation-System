...
class Dashboard(QWidget):
    ...
    def ui_Setup(self):
        ...
        # --- ADDED: Top bar for the logout button ---
        top_bar_layout = QHBoxLayout()
        top_bar_layout.addStretch(1) # Spacer to push the button to the right

        self.logout_button = QPushButton("Log Out")
        self.logout_button.setFixedWidth(100) # Give the button a fixed width
        self.logout_button.clicked.connect(self.handle_logout)
        top_bar_layout.addWidget(self.logout_button)
        
        self.layoutMain.addLayout(top_bar_layout) # Add the top bar to the main layout
        # --- END OF ADDED CODE ---
    ...
    # --- ADDED: Function to handle logging out ---
    ...
    def get_selected_room_data(self, room_name):
        # Find the first available room that matches the selected type
        for room in self.rooms_from_backend:
            api_name = room.get("room_type")
            if api_name and api_name.strip().lower() == room_name.strip().lower():
                return room  # Return the entire dictionary for the first matching room
        
        return None # Return None if no available room of that type is found

    def on_book_now(self):
        booking_payload, err = self.build_booking_payload()
        if err:
            QMessageBox.warning(self, "Invalid selection", err)
            return

        # Refresh the list of available rooms from the backend
        self.fetch_rooms_and_cache()

        # Get the full data for the selected room using the helper
        room_name = booking_payload.get("room_type")
        room_data = self.get_selected_room_data(room_name)

        if room_data is None:
             QMessageBox.warning(self, "Room Not Found", "Could not find details for the selected room. It may not be available.")
             return

        try:
            # --- CHANGE HERE: Import BookingPage ---
            from bookingPage import BookingPage
        except Exception as e:
            QMessageBox.critical(self, "Import Error",
                                 f"Could not open booking page. {e}")
            return

        # --- CHANGE HERE: Create and show an instance of BookingPage ---
        self.next_win = BookingPage(
            self.app, self.user_data, room_data)
        self.next_win.show()
        self.close()
    ...
    def on_view_history(self):
        try:
            ...
            QMessageBox.critical(self, "Import Error",
                                 f"Could not open booking history. {e}")
            return
